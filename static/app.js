...
const TIER_COLOR = { safe: new THREE.Color('#FF7A3D'), likely: new THREE.Color('#F5C26B'), review: new THREE.Color('#E8457A'), note: new THREE.Color('#6B7A99') };
const TIER_LABEL = { safe: 'Safe to remove', likely: 'Probably safe', review: 'Worth a look', note: 'Just so you know' };
const TIER_HEX = { safe: '#FF7A3D', likely: '#F5C26B', review: '#E8457A', note: '#6B7A99' };
...
function makeSage() {
  ...
  // the wisp it leaves while gliding in: motes that hang a moment in the air, then let go
  const motes = Array.from({ length: 22 }, () => {
    const m = new THREE.Mesh(new THREE.SphereGeometry(0.34, 8, 6), new THREE.MeshBasicMaterial({ color: '#C9B3FF', transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false, fog: false }));
...
function updateRuBody(dt, now) {
  ...
  if (guru.arrive < 1 && !REDUCED) {
    ...
    if (guru.emit <= 0) {
      guru.emit = 0.045;
      const t = guru.motes[guru.mote = (guru.mote + 1) % guru.motes.length];
      t.m.position.set(guru.x + (Math.random() - 0.5) * 1.6, guru.y + (Math.random() - 0.5) * 1.6, guru.z + (Math.random() - 0.5) * 1.6);
      t.m.visible = true; t.t = 0;
    }
  }
  for (const t of guru.motes) {
    if (t.t >= 1) { t.m.visible = false; continue; }
    t.t = Math.min(1, t.t + dt / 1.3);
    t.m.material.opacity = 0.45 * Math.pow(1 - t.t, 1.5);
    t.m.scale.setScalar(0.5 + t.t * 1.4); // it thins as it lets go
    t.m.position.y += dt * 1.5;           // and drifts gently upward
  }
...
const tiersOn = new Set(['safe', 'likely', 'review']); // `note` is inventory: off unless asked for
const TIER_ORDER = { safe: 0, likely: 1, review: 2, note: 3 };
const KIND_HINT = {
  ...
  archives: 'Old app builds from Xcode', models: 'Re-downloaded on demand', 'ollama-orphan': 'Listed by Ollama, weights already gone', backups: 'Manage from Finder', idlefile: 'Not opened or changed for that long', ru: 'Nominated by Ru',
};
...
const candRow = (c) => ({ key: c.path, name: c.name, is_dir: c.is_dir, size: c.size, cmd: c.remove_cmd, files: c.full_size != null && c.full_size !== c.size ? c.path : null, sub: [parentOf(c.path) || rootName, c.what, c.full_size != null && c.full_size !== c.size ? `the idle ${fmt(c.size)} of ${fmt(c.full_size)}` : ''].filter(Boolean).join(' · ') });
...
async function ruOpen(area) {
  ...
  const fresh = !guru.g;
  ruShow(true);
  if (!fresh) { guru.x += 40; guru.y += 26; guru.z -= 30; guru.vx = guru.vz = 0; }
  guru.arrive = 0;
