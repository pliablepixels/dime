...
**Di (disk)**: pick a drive or folder, Go. The map forms live while the scan runs (millions of files in seconds on a warm disk). Blocks are folders, area and height follow size, every folder carries an inset preview of its children. Colour by file type or by age. Click to explode into a folder, Esc to come back, right-click for Finder, Copy path, Hide, Send to Ru. The Cleanup tab leads with what is safe to free, in tiers (safe / probably safe / worth a look) with a plain-language reason for each item; rows unfold in place so you can tick things inside them. Sizes are what removing something actually frees: bytes another path holds through a hard link say so rather than being counted twice, and a row says when a local Time Machine snapshot will hold the space back for a day. Anything inside iCloud Drive, Dropbox, Google Drive or a photo library is measured and drawn but never suggested, because moving a file out of one of those takes it off every other device too. The idle slider narrows everything to files untouched for that long, and acting on a folder then moves only those files. Ticking adds to your **shortlist**, which is only a list; nothing moves. The shortlist is the one place anything happens, and it offers two things: **shelve** them, which moves them to `~/.dime/shelf` where they stay on the disk and can be put back at any time, or, behind a second confirmation, delete them for good. Shelving frees no space by itself; deleting from the shelf is the step that does. **Exclusions**, top right, opens a folder browser where you can pick anything you never want counted, at any time; a folder on the map takes the same thing from its right-click menu. They are kept in `~/.dime/skip.json`, apply to every root and every future scan, and the map says how many it left out so its totals are never quietly short. The scan root is watched, so changes on disk show within a couple of seconds. The last map of each root reopens instantly from a snapshot; Rescan refreshes it.
